...
import random
import time
from final.queues import BoundedQueue
from final.queues import CircularQueue
...
def dequeue_experiment(queue):
    ...
    '''
    total_time = 0.0  # the time to dequeue all the elements
...
def main():
    ...
    print(line2)
    print(str(" B que Time  | " + ' '.join(
        " {:<10} |".format(('%.2E' % Decimal(str(q)))[:10]) for q in bounded_queue_times)))
    print(line2)
